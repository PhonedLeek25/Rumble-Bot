...
async function scheduledAnnounce(client, sched_date, msg) {
    ...
    if (msdiff >= 86400000) { //24 Hours = 86,400,000 Milliseconds
        ...
        await client.channels.cache.get(channelID.announcements).send(msg + //Discord uses UNIX_TIMESTAMP() seconds! not ms.
    ...
    if (msdiff >= 3600000) { //1 Hour = 3,600,000 Milliseconds
        ...
        await client.channels.cache.get(channelID.announcements).send("Reminder! " + msg + //Discord uses UNIX_TIMESTAMP() seconds! not ms.
            `starting <t:${Math.floor(sched_date.getTime() / 1000)}:R>`);
    }
    else {
        await client.channels.cache.get(channelID.commands_and_testing).send("<@261216694901538816> critical error, in if (msdiff >= 3600000)");
    }
}
